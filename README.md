...
- **Per subscriber**: ~1KB + (queue_size × avg_message_size)
- **Per topic**: ~1KB + (replay_buffer_size × avg_message_size)
- **Bounded**: Maximum memory is predictable and configurable

### Latency

- **Subscribe/Unsubscribe**: < 1ms (per-topic lock)
- **Publish**: < 1ms (non-blocking enqueue)
- **Message delivery**: < 10ms (async send loop)
...
subscriber = Subscriber(..., max_queue_size=1000)
```
